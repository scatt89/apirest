...
CMD ["java", "-jar", "${JAVA_APP}", "--spring.datasource.url=jdbc:mysql://${MYSQL_CONTAINER}:${MYSQL-PORT}/${MYSQL_DATABASE}", "--spring.jpa.hibernate.ddl-auto=${DLL-AUTO}", "--spring.datasource.username=${USERNAME}", "--spring.datasource.password=${PASS}"]
